...
public class JWTTokenProvider {
    ...
    public String generateToken(Authentication authentication) {
        User user = (User) authentication.getPrincipal(); // object "principal" stores inside it user's information
        Date now = new Date(System.currentTimeMillis());
        Date expiryDate = new Date(now.getTime() + SecurityConstants.EXPIRATION_TIME); // time when token ends

        String userId = Long.toString(user.getId());

        //создаем объект, который будем передавать jwt
        ...
        return Jwts.builder()//import io.jsonwebtoken.*;
                .setSubject(userId)
                .addClaims(claimsMap)
                .setIssuedAt(now)
