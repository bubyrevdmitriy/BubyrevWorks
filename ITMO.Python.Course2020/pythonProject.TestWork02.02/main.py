...
#Задание 2.2
#В проекте нет необходимости во множественном наследовании(программа и так хорошо работает), поэтому применяется только абстрактный класс
# Класс mesurmentsResult выполнен абстрактным, поскольку нам не нужно создавать кземпляры данного класса
    ...
    def __add__(self, other):
        if isinstance(other, self.__class__):
            if (self.getNumberOfMesurments()+other.getNumberOfMesurments()>20):
                return mesurmentsResultCharlier(self.getNumberOfMesurments()+other.getNumberOfMesurments(),self.getMesurmentsArrayEnter()+other.getMesurmentsArrayEnter(),self.getInf())
            else:
...
class mesurmentsResultRomanovsky(mesurmentsResult, Mixin1, Mixin2):
    ...
    def __add__(self, other):
        if isinstance(other, self.__class__):
            if (self.getNumberOfMesurments()+other.getNumberOfMesurments()<=20):
                return mesurmentsResultRomanovsky(self.getNumberOfMesurments()+other.getNumberOfMesurments(),self.getMesurmentsArrayEnter()+other.getMesurmentsArrayEnter(),self.getInf())
            else:
    ...
    def printClass(self):
        ...
        print("i".ljust(10), "Array".ljust(20),  "Deviations".ljust(20), "DeviationsSquared".ljust(20),  "Beta".ljust(20),  "Result".ljust(20))
        print("---------------------------------------------------------------------------------------------------------------")
